...
class TestHashkeyPerpetualAPIOrderBookDataSource(IsolatedAsyncioWrapperTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.connector = HashkeyPerpetualDerivative(
        ...
        self.ob_data_source = HashkeyPerpetualAPIOrderBookDataSource(
            trading_pairs=[self.trading_pair],
    ...
    async def test_request_order_book_snapshot(self, mock_api):
        ...
        tradingrule_url = web_utils.rest_url(CONSTANTS.EXCHANGE_INFO_URL)
    ...
    async def test_get_new_order_book(self, mock_api):
        ...
        self.assertEqual(1000, bid_entries[0].amount)
        ...
        self.assertEqual(1000, ask_entries[0].amount)
        self.assertEqual(25981.76, ask_entries[1].price)
        self.assertEqual(2000, ask_entries[1].amount)
    ...
    @patch("aiohttp.ClientSession.ws_connect", new_callable=AsyncMock)
    @patch("hummingbot.connector.derivative.hashkey_perpetual.hashkey_perpetual_api_order_book_data_source.HashkeyPerpetualAPIOrderBookDataSource._time")
